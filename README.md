...
- диагностировать, почему в Certificate Manager долго не выпускается или не перевыпускается сертификат;
...
- Если скрипт случайно запущен напрямую через `python3` или shebang без этих зависимостей, он сам повторит запуск через `uv run`; при отсутствии `uv` вернет структурированную ошибку с подсказкой.
